...
edfapi_version = '4.2.762.0 MacOS X Aug 12 2024';
header_file = fullfile(folder, "edfapi.framework", "Headers");
...
fid = fopen(fullfile(header_file, 'edf_data.h'));
...
fid = fopen(fullfile(fileparts(folder), 'source', 'edf2mex.h'), 'w');

fprintf(fid,'\n\nchar const * build_version = "%s";\n\n', edfapi_version);
...
for i = 1:length(rectype)
    ...
    if ~isfield(RecTypes.(rectype{i}),'vars')
        continue
    end
    ...
    % Create a structure that contains data type arrays (which can be passed to mex)

    fprintf(fid,'\n\n struct \t%stype\n{\n\tconst char * fieldnames[%i];',rectype{i}, length(recfields));
    if contains(lower(rectype{i}),'sample')
